...
class Scene
    attr_accessor :entities, :background, :camera
    def initialize()
        @entities = []
        @camera = Camera.new()
        @camera.setup()
    ...
    def scene_event_handler()
        ...
        @entities.each do |entity|
            entity.event_handler() if entity.components.include?(Controllable)
        end
        event_handler()
    end
...
#=end
    ...
    def del_entity(id)
        @entities[id].texture.cleanup() unless @entities[id].texture == nil
        @entities[id] = nil
    end
