...
def on_right_click(pr) = $gb_var[:event_handler].on_click(3, pr);

def on_middle_click(pr) = $gb_var[:event_handler].on_click(2, pr);

def on_left_click(pr) = $gb_var[:event_handler].on_click(1, pr);

def on_move(pr) = $gb_var[:event_handler].on_move(pr);

def on_wheel(pr) = $gb_var[:event_handler].on_wheel(pr);

def mouse() = $gb_var[:event_handler].mouse();
